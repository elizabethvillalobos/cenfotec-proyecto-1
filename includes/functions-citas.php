<?php
	...
	function getSolicitudesUsuario($idUsuario) {
		...
		if($row['nombre']=="Estudiante"){
			$query = "SELECT tc.id,tc.idSolicitado,tu.nombre, tu.apellido1,tc.idSolicitante,tc.fechaInicio FROM `tcitas` AS tc INNER JOIN `tusuarios` AS tu ON tc.idSolicitado = tu.id WHERE idSolicitante='$idUsuario' AND tc.esCita='0'  AND (tc.estado=1 OR tc.estado=2) ORDER BY tc.id ASC";
